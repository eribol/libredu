...
use crate::{send_mail::send_mail, tokio::sync::RwLock, user::signin};
use moon::{futures::TryStreamExt, Lazy, *};
...
pub async fn get_user(auth: &str) -> redis::RedisResult<i32> {
...
pub async fn set_user(id: i32, auth_token: &AuthToken) -> redis::RedisResult<()> {
    ...
    set_session_pg(id, &auth_token.clone().into_string())
        .await
        .expect("pg hatası");
    ...
    match d {
        Ok(domain) => {
            let html = create_html(
                domain,
                user.email.clone(),
                auth_token.clone().into_string().trim().to_string(),
            );
            send_mail(user.email, html, String::from("Üyelik Onayla")).await
        }
        Err(_e) => DownMsg::SigninError("e posta yok".to_string()),
    }
}
