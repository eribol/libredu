use crate::{
    connection::{
        self, forget_password, get_user, reset_password,
        school::{add_school, get_school, get_school_api, update_school},
    },
    user::{self, is_user_exist},
};
use moon::*;
use shared::{
    msgs::{lectures::LecturesUpMsg, teachers::TeacherUpMsgs, timetables::TimetablesUpMsgs},
    *,
};
...
pub async fn up_msg_handler(req: UpMsgRequest<UpMsg>) {
    ...
    let down_msg = match up_msg {
        ...
        UpMsg::ForgetPassword { email } => match is_user_exist(&email).await {
            Ok(_) => forget_password::forget_password(email).await,
            Err(_) => DownMsg::ResetPassword,
        },
        UpMsg::ResetPassword(form) => reset_password::reset_password(form).await,
        UpMsg::Signin { form } => match is_user_exist(&form.email).await {
        ...
        UpMsg::Register(token, email) => connection::get_register(token, email).await,
        ...
        UpMsg::GetSchoolApi(id) => {
            if let Some(_auth_token) = auth_token {
                // let manager = get_user(&auth_token.into_string()).await;
                let api = get_school_api(id).await;
        ...
        UpMsg::Classes(class_msg) => classes_msgs(class_msg, auth_token).await,
            ...
            if let Some(auth) = auth_token {
                ...
                if let DownMsg::GetSchool { id, .. } = school_msg {
                    match tt_msg {
                        ...
                        TimetablesUpMsgs::AddTimetable(form) => {
                            timetables::add_timetable(form, id).await
                        }
                        TimetablesUpMsgs::DelTimetable(group_id) => {
                            timetables::del_timetable(id, group_id).await
                        }
                        TimetablesUpMsgs::GetSchedules(group_id) => {
                            timetables::get_schedules(group_id).await
                        }
                        TimetablesUpMsgs::UpdateSchedules(schedules) => {
                            timetables::update_schedules(schedules).await
                        }
                        TimetablesUpMsgs::UpdateTimetable(form) => {
                            timetables::update_timetable(form, id).await
                        } //TeacherUpMsgs::DelTeacher(user_id) => del_teacher(user_id, id).await
                    }
                } else {
                    school_msg
            ...
            if let Some(auth) = auth_token {
                ...
                if let DownMsg::GetSchool { id, .. } = school_msg {
                    match l_msg {
                        ...
                        LecturesUpMsg::UpdateLecture(form) => {
                            lectures::update_lecture(id, form).await
                        }
        ...
        UpMsg::Admin(a_msg) => admin_msgs(a_msg, auth_token).await,
        UpMsg::Messages(msg) => messages::message(msg, auth_token).await,
    };
