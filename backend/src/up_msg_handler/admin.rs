...
async fn run_msg(msg: AdminUpMsgs)->AdminDownMsgs{
    match msg{
        ...
        AdminUpMsgs::SearchSchool(group_id) => crate::connection::admin::get_classes(group_id).await,
        AdminUpMsgs::GetTeachers(school_id) => crate::connection::admin::get_teachers(school_id).await,
        AdminUpMsgs::GetClasses(group_id) => crate::connection::admin::get_classes(group_id).await,
        AdminUpMsgs::GetTimetables(group_id) => crate::connection::admin::get_timetables(group_id).await,
        AdminUpMsgs::GetActivities(group_id) => crate::connection::admin::get_activities(group_id).await,
        AdminUpMsgs::GetClassesLimitations(group_id) => crate::connection::admin::get_classes_limitations(group_id).await,
        AdminUpMsgs::GetTeachersLimitations(group_id) => crate::connection::admin::get_teachers_limitations(group_id).await,
