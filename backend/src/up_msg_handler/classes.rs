...
pub async fn add_class(id: i32, form: AddClass) -> DownMsg {
    ...
    if let Some(_) = school_group.try_next().await.unwrap() {
        let mut row = sqlx::query(
            r#"insert into classes(grade, branch, school, group_id) 
                        values($1, $2, $3, $4)
                        returning id, grade, branch, group_id"#,
        )
        .bind(&form.grade)
        .bind(&form.branch)
        .bind(id)
        ...
        if let Ok(query) = row.try_next().await {
            if let Some(class) = query {
                ...
                DownMsg::Classes(ClassDownMsgs::AddClassError(
                    "Failed to add class".to_string(),
                ))
            }
        } else {
...
pub async fn get_classes(id: i32) -> DownMsg {
    let db = POSTGRES.read().await;
    let mut row = sqlx::query(
        r#"select id, grade, branch, group_id, short_name from classes
                        where school = $1"#,
    )
    .bind(id)
    .fetch(&*db);
    ...
    while let Some(class) = row.try_next().await.unwrap() {
        let c = Class {
            id: class.try_get("id").unwrap(),
            name: class.try_get("short_name").unwrap(),
            short_name: class.try_get("short_name").unwrap(),
            branch: class.try_get("branch").unwrap(),
            grade: class.try_get("grade").unwrap(),
            group_id: class.try_get("group_id").unwrap(),
        };
...
pub async fn del_class(class_id: i32, school_id: i32) -> DownMsg {
    let db = POSTGRES.read().await;
    let mut row = sqlx::query(r#"delete from classes where id = $1 and school = $2 returning id"#)
        .bind(class_id)
        .bind(school_id)
        .fetch(&*db);
...
pub async fn del_class_acts(id: i32) {
    //use moon::tokio_stream::StreamExt;
    let db = POSTGRES.read().await;
    let _ = sqlx::query(r#"delete from activities where $1 = any(classes)"#)
        .bind(id)
        .execute(&*db)
        .await;
}
