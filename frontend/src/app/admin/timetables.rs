...
fn classes_page()->impl Element{
    Column::new()
        ...
        .label_signal(
            fix_class_lim()
            .signal()
            .map_bool(|| "Sınıf Kısıtlamaları Düzeltildi", || "Sınıf Kısıtlamaları Düzeltilmedi")
        )
        .s(
            Font::new()
            .weight_signal(
...
fn teachers_page()->impl Element{
    Label::new()
        .label_signal(
            fix_teach_lim()
            .signal()
            .map_bool(|| "Öğretmen Kısıtlamaları Düzeltildi", || "Öğretmen Kısıtlamaları Düzeltilmedi")
        )
        .s(
            Font::new()
            .weight_signal(
...
fn class_lim_check(class: Class)-> bool{
    ...
    let c_lims = lims.iter().filter(|l| l.class_id == class.id.clone()).collect::<Vec<&ClassLimitation>>();
...
fn teacher_lim_check(teacher: Teacher)-> bool{
    ...
    let c_lims = lims.iter().filter(|l| l.user_id == teacher.id.clone()).collect::<Vec<&TeacherLimitation>>();
    let mut shifted = false;
    for i in 1..=7{
        if let Some(c_l) = c_lims.iter().find(|cl| cl.day == i){
            if c_l.hours.len() < t_len as usize{
...
fn act_check(act: Activity)-> bool{
    ...
    let tchrs = teachers().lock_mut().to_vec();
    let t = act.teachers.iter().all(|t| tchrs.iter().any(|t2| t2.id == *t));
    let c = act.classes.iter().all(|c| clss.iter().any(|c2| c2.id == *c));
    t && c
}
fn acts_check(){
