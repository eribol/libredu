...
fn modal_header() -> impl Element {
    Row::new()
        .item(
            Label::new()
                .label_signal(class_modal().signal_cloned().map_option(
                    |class| (class.kademe + &class.sube).into_cow_str(),
                    || "".into_cow_str(),
                ))
                .s(Align::center()),
        )
...
pub fn open_modal(class: Class) {
    ...
    visibile().set(modal);
    class_modal().set(Some(class))
}

fn close_modal() {
    let modal = Modal::default();
    visibile().set(modal);
    class_modal().set(None)
}
#[static_ref]
pub fn class_modal() -> &'static Mutable<Option<Class>> {
    Mutable::new(None)
}
...
#[static_ref]
pub fn limitations() -> &'static MutableVec<ClassLimitation> {
    get_limitations();
...
fn get_limitations() {
    Task::start(async {
        let id = class_modal().get_cloned().unwrap().id;
...
fn get_activities() {
    Task::start(async {
        let id = class_modal().get_cloned().unwrap().id;
        let msg = shared::UpMsg::Classes(shared::msgs::classes::ClassUpMsgs::GetActivities(id));
        match crate::connection::connection().send_up_msg(msg).await {
            Err(_error) => {}
            Ok(_msg) => (), //println!("{:?}", msg),
        }
