...
fn name_view() -> impl Element {
    Column::new()
        ...
        .item_signal(name_error().signal_cloned().map_some(|s| {
            Label::new()
                .label(s)
                .s(Align::center())
                .s(Font::new().weight(FontWeight::Light).color(color!("red")))
        }))
}
fn short_name_view() -> impl Element {
    Column::new()
        .item(
            Label::new()
                .label_signal(t!("lecture-shortname"))
                .s(Align::center()),
        )
        .item(
            text_inputs::default()
        ...
        .item_signal(short_name_error().signal_cloned().map_some(|s| {
            Label::new()
                .label(s)
                .s(Align::center())
                .s(Font::new().weight(FontWeight::Light).color(color!("red")))
...
fn add() -> impl Element {
    ...
    Button::new()
        ...
        .label_signal(
            selected_lecture()
                .signal()
                .map_option(|_| t_s!("update"), || t_s!("add")),
        )
}
    ...
    Row::new()
        ...
        .items_signal_vec(lectures().signal_vec_cloned().map(|r| {
            ...
            Column::new()
                ...
                .s(Width::exact(100))
                .s(Height::exact(50))
                .s(Background::new().color_signal(is_selected(r.id).map_true(|| color!("blue"))))
                ...
                .item_signal(del_signal(r.id).map_bool(
                    move || {
                        crate::modals::del_modal_all(
                            &r.id.to_string(),
                            UpMsg::Lectures(LecturesUpMsg::DelLecture(r.id)),
                        )
                        .into_raw()
                    },
