use super::classes::{selected_timetable_hour, timetables};
use crate::{
    connection::send_msg,
...
use shared::{
    models::timetables::{AddTimetable, Timetable, TimetableSchedules},
    UpMsg,
};
use zoon::*;
    ...
    Row::new()
        ...
        .items_signal_vec(timetables().signal_vec_cloned().map(|tt| {
            ...
            Column::new()
                ...
                .s(Background::new().color_signal(is_selected(tt.id).map_true(|| color!("blue"))))
                .s(Borders::all_signal(a.signal().map_bool(
                    || Border::new().width(1).color(color!("blue")).solid(),
                    || Border::new().width(1).color(color!("blue")).solid(),
                )))
                ...
                .update_raw_el(|raw| raw.attr("title", &format!("{}", tt.name)))
                .item(Button::new().label(tt.name))
                .item_signal(crate::modals::del_signal(tt.id).map_bool(
                    move || {
                        crate::modals::del_modal_all(
                            &tt.id.to_string(),
                            UpMsg::Timetables(TimetablesUpMsgs::DelTimetable(tt.id)),
                        )
                        .into_raw()
                    },
                    move || delete_view(tt.id).into_raw(),
                ))
        }))
}
    ...
    Row::new()
        ...
        .item(
            Column::new()
                ...
                .item(
                    Label::new()
                        .s(Borders::all(Border::new().width(1).color(color!("blue"))))
                        .s(Height::exact(25))
        ...
        .item(
            Column::new()
                ...
                .items(s.starts.iter().enumerate().map(|ss| {
                    TextInput::new()
                        .id("a")
                        .s(Height::exact(25))
                        .s(Borders::all(Border::new().width(1).color(color!("blue"))))
                        .text(ss.1.format("%H:%M").to_string())
                        .update_raw_el(|raw| raw.attr("type", "time"))
        ...
        .item(
            Column::new()
                ...
                .items(s.ends.iter().enumerate().map(|ss| {
                    TextInput::new()
                        .id("a")
                        .s(Height::exact(25))
                        .s(Borders::all(Border::new().width(1).color(color!("blue"))))
...
fn is_selected(id: i32) -> impl Signal<Item = bool> {
    selected_timetable()
        .signal_cloned()
        .map_option(move |s| s.id == id, || false)
        .dedupe()
}
...
fn create_selected(tt: Timetable) {
    ...
    selected_timetable_hour()
        .lock_mut()
        .replace(vec![0; tt.hour as usize]);
    get_schedules(tt.id);
}
fn clear_data() {
    hide().set(false);
